		...
		for (int i = 0; i < gameOutline.length; i++) {
			for (int j = 0; j < (gameOutline[i].length); j++) {
				if (gameOutline[i][j].equals("A")) {
					...
					if (gameOutline[i - 1][j] == "_" || gameOutline[i - 1][j].equals("|")) {
						hitwall = true;
					}
				}
			}
		...
		for (int i = 0; i < gameOutline.length; i++) {
			for (int j = 0; j < (gameOutline[i].length); j++) {
				if (gameOutline[i][j].equals("A")) {
					...
					if (gameOutline[i][j + 3].contentEquals("|") || gameOutline[i][j + 2].contentEquals("|")
							|| gameOutline[i][j + 1].contentEquals("|")) {
						hitwall = true;
					}
				}
			}
		...
		for (int i = 0; i < gameOutline.length; i++) {
			for (int j = 0; j < (gameOutline[i].length); j++) {
				if (gameOutline[i][j].equals("A")) {
					...
					if (gameOutline[i + 1][j].contentEquals("_") || gameOutline[i + 1][j].equals("|")) {
						hitwall = true;
					}
				}
			}
